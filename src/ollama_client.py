...
class OllamaClient:
    ...
    def generate(self, 
        ...
        # ── Thinking-model num_predict bump ──
        # Ollama-hosted thinking models (qwen3.5, qwen3-next-thinking, deepseek-r1,
        # qwq, kimi-k2-thinking, etc.) spend most of their token budget on
        # internal reasoning tokens. A 2000-token default will not leave enough
        # room for a useful final answer. The thinking-models registry classifies
        # known reasoners; for those we bump num_predict to at least 16384.
        #
        # Small models (gemma4:e4b, etc.) are also listed in the registry as a
        # capacity fix — they often truncate to empty output on long agentic
        # prompts unless num_predict is bumped. We additionally lower the
        # temperature for small models so they commit more decisively to a
        # verdict instead of dithering into empty output.
        is_small_model_capacity_fix = False
        try:
            from src.runtime.thinking_models import (
                is_thinking_model, THINKING_MODEL_MAX_TOKENS,
            )
            if is_thinking_model(used_model):
                thinking_min = max(16384, THINKING_MODEL_MAX_TOKENS // 2)
                if used_max_tokens < thinking_min:
                    self.logger.info(
                        "Thinking model '%s' detected, bumping num_predict %d -> %d",
                        used_model, used_max_tokens, thinking_min,
                    )
                    used_max_tokens = thinking_min
                # Detect small-model capacity-fix entries (gemma4:e4b family).
                # These are listed in the registry but aren't real reasoners;
                # they need a deterministic decode to avoid blanking out.
                model_lower = used_model.lower()
                if "e4b" in model_lower or "gemma4:e" in model_lower:
                    is_small_model_capacity_fix = True
        except ImportError:
            # thinking_models registry not available — use original max_tokens
            pass

        # Lower temperature for small-model capacity fix path. Empirically,
        # gemma4:e4b drops from ~50% to <30% blank rate when temperature is
        # cut from 0.7 to 0.1 because the decoder commits to a verdict instead
        # of sampling itself into an empty completion.
        if is_small_model_capacity_fix and used_temp > 0.2:
            self.logger.info(
                "Small-model capacity fix '%s': lowering temperature %.2f -> 0.1",
                used_model, used_temp,
            )
            used_temp = 0.1
