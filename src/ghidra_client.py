...
class GhidraMCPClient:
    ...
    def __init__(self, config: GhidraMCPConfig, ollama_client=None):
        ...
        """
        self.config = config
        # Bearer auth token for the Ghidra HTTP server (added 2026-04-08).
        # Resolved by reading the sidecar file the Java server writes at
        # startup, or from AGENT_G_GHIDRA_AUTH_TOKEN env var. If neither is
        # present, requests are sent unauthenticated (compat mode for old
        # servers without the auth filter).
        self._auth_token = self._resolve_auth_token(config)
        default_headers = {}
        if self._auth_token:
            default_headers["Authorization"] = f"Bearer {self._auth_token}"
        self.client = httpx.Client(timeout=config.timeout, headers=default_headers)
    ...
    @staticmethod
    def _resolve_auth_token(config) -> Optional[str]:
        """Resolve the Ghidra HTTP bearer token.

        Order of precedence:
          1. AGENT_G_GHIDRA_AUTH_DISABLE=1 → return None (unauth mode)
          2. AGENT_G_GHIDRA_AUTH_TOKEN env var
          3. Sidecar file the Java server wrote at startup:
             ${TMPDIR}/agent_g_ghidra_token_<port>.txt
          4. None → unauth mode (compat with pre-auth servers)
        """
        import os
        import tempfile
        if os.environ.get("AGENT_G_GHIDRA_AUTH_DISABLE") == "1":
            return None
        env_tok = os.environ.get("AGENT_G_GHIDRA_AUTH_TOKEN")
        if env_tok:
            return env_tok
        try:
            parsed = urlparse(str(config.base_url))
            ...
            logger.debug("Could not read Ghidra auth sidecar: %s", e)
        return None
